...
class ProjectStacksRepository:
        ...
        try:
            with pool.connection() as conn:
                with conn.cursor() as db:
                    db.execute(
                        """
                        SELECT id, project_id, tech_stacks_id
                        FROM project_stacks
                        ORDER BY id;
                        """
                    )
    ...
    def get_project_stack(self, project_stack_id: int) -> List[ProjectStackOut]:
    ...
    def create_project_stack(self, project_stack: ProjectStackIn) -> ProjectStackOut:
        with pool.connection() as conn:
            with conn.cursor() as db:
                result = db.execute(
                    """
                    INSERT INTO project_stacks
                        (project_id, tech_stacks_id)
                    VALUES
                        (%s, %s)
                    RETURNING id;
                    """,
    ...
    def update_project_stack(self, project_stack_id: int, project_stack: ProjectStackIn) -> Union[ProjectStackOut, Error]:
        try:
            with pool.connection() as conn:
                with conn.cursor() as db:
                    db.execute(
                    ...
                    return self.project_stack_in_to_out(project_stack_id, project_stack)
        except Exception as e:
            print(e)
